...
public class User2Service {
    ...
    public ResponseEntity<?> insertUser2(User2DTO user2DTO){
        /*
        JPA save()는 삽입, 수정을 동시에 할 수 있는 메소드이기 때문에
        삽입을 수행하고자 할 경우에는 먼저 existsById()로 존재여부를 확인하고
        save()를 수행하면 됨
     */
        ...
        if(user2Repository.existsById(user2DTO.getUid())){
            ...
            return ResponseEntity
                    ...
                    .body(user2DTO.getUid() + "already exit");
    ...
    public ResponseEntity<?> updateUser2(User2DTO user2DTO){
        ...
        if(user2Repository.existsById(user2DTO.getUid())){
            ...
            // 사용자가 존재하지 않으면 NOT_FOUND 응답 데이터와 user not found 메세지
            return ResponseEntity
                        .status(HttpStatus.NOT_FOUND)
                        .body("user not found");
        }
